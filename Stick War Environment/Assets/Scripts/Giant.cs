...
public class Giant : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (Vector2.Distance(transform.position, toMove) < 3f && (((gv.team1 != 2 && gv.team1 != 1 && gv.team1 != 4 && target == "Team2") || (gv.team2 != 2 && gv.team2 != 1 && gv.team2 != 4 && target == "Team1"))))
        {
            if (!isAttacking)
            {
    ...
    public bool Attack(float dmg)
    {
        ...
        if (GetComponent<SpriteRenderer>().flipX && left.IsTargetWithinRange(enemy.gameObject))
        {
            ...
            enemies = left.IsTagWithinRangeList(target, 5);
        ...
        {
            List<GameObject> enemies = new List<GameObject>();
            enemies = right.IsTagWithinRangeList(target, 5);
